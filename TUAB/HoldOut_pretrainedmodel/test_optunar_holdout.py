...
save_path = os.environ.get("TUAB_SAVE_DIR", str(PROJECT_ROOT / "test_classification" / "result_optuna_holdout"))
save_path = os.path.join(save_path, "")
if not os.path.exists(save_path):
    os.makedirs(save_path)

# 固定パラメータ
# Max epochs per trial. Lower it (e.g. TUAB_MAX_EPOCHS=40) to fit the whole study in the SLURM time limit.
epoch = int(os.environ.get("TUAB_MAX_EPOCHS", "100"))
seed = 42
...
def load_data():
    data_dir = os.environ.get("TUAB_DATA_DIR", str(PROJECT_ROOT / "TUAB_data"))
...
def visualize_electrode_importance(importance_vector, acc):
    ...
    plt.tight_layout()
    plt.savefig(f"{save_path}electrode_importance_val.png")
    plt.close()

# --- 学習処理 ---
def run_trial(params, X_train, y_train, X_val, y_val, X_eval, y_eval, num_classes, trial_number=0, trial=None):
    set_seed(seed)
    ...
    early_stop_patience = int(os.environ.get("TUAB_EARLY_STOP_PATIENCE", "20"))
    epochs_no_improve = 0
    ...
    # Hard per-trial wall-clock cap (seconds). Guarantees each trial (and the final
    # best-params training) ends in bounded time so the whole run fits the SLURM limit.
    # Unset/empty -> disabled (original behaviour, keeps `bash 1.sh` working unchanged).
    _ttb = os.environ.get("TUAB_TRIAL_TIME_BUDGET", "")
    trial_time_budget = int(_ttb) if _ttb.strip() else None
    ...
    for i in range(epoch):
        ...
        if epoch_val_combined_score > best_val_combined_score:
            ...
            # The shape of tensors in current_epoch_elec_imp is [layers, batch, tokens].
            # To concatenate them correctly across all batches, we must use dim=1 (the batch dimension).
        ...
        # --- Optuna pruning: report intermediate score and kill under-performing trials early ---
        if trial is not None:
            trial.report(epoch_val_combined_score, i)
            if trial.should_prune():
                print(f"  Trial {trial.number} pruned at epoch {i+1} (Val Comb: {epoch_val_combined_score:.4f}).")
                raise optuna.TrialPruned()

        # --- Hard time cap: stop this trial early but keep the best model found so far ---
        if trial_time_budget is not None and (time.time() - trial_start_time) > trial_time_budget:
            print(f"  Time budget of {trial_time_budget}s reached; stopping trial at epoch {i+1}.")
            stopped_epoch = i + 1
            break
    ...
    return {
        ...
        'train_loss_history': train_loss_history[:stopped_epoch],
        'val_loss_history': val_loss_history[:stopped_epoch],
        'train_acc_history': train_acc_history[:stopped_epoch],
        'val_acc_history': val_acc_history[:stopped_epoch],
...
def objective(trial):
    ...
    res = run_trial(params, X_train, y_train, X_val, y_val, X_eval, y_eval, num_classes, trial_number=trial.number, trial=trial)
...
def evaluate_with_best_params(best_params):
    ...
    try:
        plot_epochs = range(1, stopped_epoch + 1)
        plt.figure(figsize=(10, 5))
        plt.plot(plot_epochs, train_loss_history, label='Training Loss')
        plt.plot(plot_epochs, val_loss_history, label='Validation Loss', linestyle='--')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        ...
        plt.figure(figsize=(10, 5))
        plt.plot(plot_epochs, train_acc_history, label='Training Accuracy')
        plt.plot(plot_epochs, val_acc_history, label='Validation Accuracy', linestyle='--')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy (%)')
        plt.ylim(0, 100) 
        plt.legend(); plt.grid(True)
...
if __name__ == '__main__':
    ...
    processes_per_gpu = int(os.environ.get("PROCESSES_PER_GPU", "5"))
    n_jobs = int(os.environ.get("OPTUNA_N_JOBS", max(1, num_gpus * processes_per_gpu)))
    n_trials = int(os.environ.get("OPTUNA_N_TRIALS", "40"))
    
    # Optuna wall-clock budget (seconds). Set below the SLURM -t limit so the study exits
    # gracefully and writes the best params instead of being hard-killed. 0/empty = no timeout.
    optuna_timeout = os.environ.get("OPTUNA_TIMEOUT", "")
    optuna_timeout = int(optuna_timeout) if optuna_timeout.strip() else None

    # Prune under-performing trials early so the whole study fits in the time budget.
    pruner = optuna.pruners.MedianPruner(
        n_startup_trials=int(os.environ.get("OPTUNA_PRUNER_STARTUP_TRIALS", "5")),
        n_warmup_steps=int(os.environ.get("OPTUNA_PRUNER_WARMUP_STEPS", "10")),
    )

    print(f"Using {num_gpus} GPUs. Running {n_jobs} parallel trials ({processes_per_gpu} trials per GPU), "
          f"n_trials={n_trials}, timeout={optuna_timeout}, max_epochs={epoch}.")

    study = optuna.create_study(
        direction="maximize",
        study_name=os.environ.get("OPTUNA_STUDY_NAME", "TUEV_Hyperparameter_Optimization_Holdout_Expanded"),
        storage=os.environ.get("OPTUNA_STORAGE", "sqlite:///optuna_study_holdout_expanded.db"),
        load_if_exists=True,
        pruner=pruner
    )
    study.optimize(objective, n_trials=n_trials, n_jobs=n_jobs, timeout=optuna_timeout)
