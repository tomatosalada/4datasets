...
class Config:
    def __init__(self):
        # モデルの設定
        self.hidden_size = 32        # 隠れ層の次元数 (optuna trial 35)
        self.num_atention_heads = 16   # アテンションヘッドの数 (optuna trial 35)
        self.num_hidden_layers = 4    # Transformer Encoder層の数 (optuna trial 35)
        self.intermediate_size = 128  # FeedForwardネットワークの中間層の次元数 (hidden_size*4)
        self.hidden_dropout_prob = 0.12764981444263288 # ドロップアウト率 (optuna trial 35)
        self.num_embedding = 16       # TUEVは16チャンネル
        self.num_labels = 6           # TUEVは6クラス
...
class Embeddings(nn.Module):
    def __init__(self, config, gnn_heads=2, gnn_dropout=0.30701473066919766):
        super().__init__()
        self.config = config
        self.gnn_dropout = gnn_dropout
        self.electrode_embedding = nn.Parameter(torch.randn(1, 16, config.hidden_size))
    ...
    def forward(self, x, edge_index):
        ...
        adj = to_dense_adj(edge_index, max_num_nodes=16)[0] 
...
class gcn_select_net(nn.Module):
    def __init__(self, num_classes=6, hidden_size=32, num_hidden_layers=4,
                 transformer_dropout=0.12764981444263288, cnn_dropout=0.19416740168749858, gnn_dropout=0.30701473066919766,
                 num_attention_heads=16, gnn_heads=2, cnn_out_channels=32):
        ...
        # TUAB用の16チャンネルグラフを読み込む。
        # 実行マシンに依存しないよう、環境変数 or リポジトリ相対パスで解決する。
        # (このファイルは <repo>/select_channel/ にあるため、親ディレクトリが <repo>)
        _repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self.graph_path = os.environ.get(
            "TUAB_GRAPH_PATH",
            os.path.join(_repo_root, "GNN", "graph_path",
                         "common_graph_edge_index_mi_0.2_50_tuab_all.pt")
        )
...
if __name__ == '__main__':
    # TUEV ダミーデータ: (Batch, Time, Freq, Channel) = (100, 10, 5, 16)
    input = torch.rand((100, 10, 5, 16))
    net = gcn_select_net(num_classes=6)
