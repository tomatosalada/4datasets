...
class GraphTransformerLayer(nn.Module):
    def __init__(self, in_features, out_features, num_nodes=18, heads=4, concat=True, dropout=0.3):
        ...
        # Learnable Bias (固定グラフの補正用: num_nodes固定)
        # 任意のつながりを学習できるようにする
        self.bias = nn.Parameter(torch.zeros(1, heads, num_nodes, num_nodes)) 
...
class DenseGAT3(nn.Module):
    def __init__(self, in_channels, out_channels, num_nodes=18, heads=4, dropout=0.3):
        super().__init__()
        head_dim = out_channels // heads
        
        # GraphTransformerLayer に置き換え
        self.conv1 = GraphTransformerLayer(in_channels, head_dim, num_nodes=num_nodes, heads=heads, concat=True, dropout=dropout)
        self.conv2 = GraphTransformerLayer(out_channels, out_channels, num_nodes=num_nodes, heads=1, concat=False, dropout=dropout)
