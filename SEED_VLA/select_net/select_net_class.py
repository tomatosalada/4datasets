...
import numpy as np
from GNN.GCN import GCN, GAT, DenseGAT, DenseGAT2, DenseGAT3
...
class Embeddings(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.electrode_embedding = nn.Parameter(torch.randn(1, 18, config.hidden_size))
        
        # カスタムDenseGATを使用
        self.gat3 = DenseGAT3(config.hidden_size, config.hidden_size)
        
    def forward(self, x, edge_index):
        batch_size = x.size(0)
        device = x.device
        
        adj = to_dense_adj(edge_index, max_num_nodes=18)[0] 
...
class MultiScalseTemporalConv(nn.Module):
    def __init__(self, in_features, out_channels_per_path):
        super().__init__()
    ...
    def forward(self, x):
        ...
        out_short = F.dropout(out_short, p=0.3, training=self.training)
        #print(out_short.shape)
        out_mid = F.relu(self.conv_mid(x))
        out_mid = F.dropout(out_mid, p=0.3, training=self.training)
        #print(out_mid.shape)
        out_long = F.relu(self.conv_long(x))
        out_long = F.dropout(out_long, p=0.3, training=self.training)
        #print(out_long.shape)
        out_all = F.relu(self.conv_all(x))
        out_all = F.dropout(out_all, p=0.3, training=self.training)
        #print(out_all.shape)
...
class gcn_select_net(nn.Module):
    def __init__(self, num_classes = 3):
        ...
        self.conv_time = MultiScalseTemporalConv(1, 16)

        self.Atten = sfAttention(in_channels=64) 

        self.conv2d = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=(3, 3), padding=(1, 1))
        # 第2引数は affine ではなく eps。False(=0) を渡すと新しい PyTorch では
        # 学習時に "batch_norm eps must be positive" で落ちるため既定値に戻した。
        # eps 0 -> 1e-5 で既存モデルの予測は 0.000% しか変わらない（実測）。
        self.bm2d = nn.BatchNorm2d(32)
        self.global_pool = nn.AdaptiveMaxPool2d((4, 2))
        self.conv2d2 = nn.Conv2d(in_channels = 32, out_channels=16, kernel_size=(3, 3), padding=(1, 1))
        self.bm2d2 = nn.BatchNorm2d(16)
        self.global_pool2 = nn.AdaptiveAvgPool2d((2, 2))
 
        self.linear1 = nn.Linear(256, 128)
        self.linear2 = nn.Linear(128, 64)

        self.graph_path = "/mnt/data/toshiki.ohno/EEG_fatigue/EEG_VLA/GNN/graph_path/common_graph_edge_index_full_18_spatial.pt"
    ...
    def forward(self, x):
        #[batch、時系列、周波数、特徴量]→x=[100, 16, 5, 18]
        batch_size, seq_len, freq_len, node_dim = x.shape  # 各次元のサイズを取得

        x = x.permute(0, 3, 1, 2) #[100, 18, 16, 5]
        x = x.reshape(-1, seq_len, freq_len)
        x = x.unsqueeze(1) #[1800, 1, 16, 5]
        x_time = self.conv_time(x) #[1800, 96, 16, 5]
        ...
        x_atten = F.dropout(x_atten, p=0.2, training=self.training)
...
if __name__ == '__main__':
    input = torch.rand((100, 16, 5, 18))
    net = gcn_select_net()
    
    output, attention_weights, electrode_attention, atten1, atten3 = net(input)
