"""全20名を学習候補にする電極数探索（Optuna教師モデル版）。
...
RESEARCH_BASE_DIR = ('/mnt/data/toshiki.ohno/EEG_fatigue/EEG_VLA/'
                     ...
                     'results_bestnum_paper2class_eeggate_optunar/')
...
# DE特徴の周波数帯数。入力は [N, 16(時間窓), 5(帯域), 18(電極)]
NUM_BANDS = 5

# 固定チャネルは設けず、全18chを可変チャネルとして扱う (電極数のみを変える)
...
def load_subjects(subject_ids, normalize):
    """paper_2class から指定被験者を読み込んで連結する。"""
    xs, ys = [], []
...
def teacher_outputs(cheat_sheet_model, x, mode):
    ...
    if mode == 'cls':
        teacher_imp = cls_row_importance(attn_w)                 # [B, 18]
    else:
...
def teacher_cache_path(cache_dir, mode, target, normalize):
    return os.path.join(
        cache_dir,
        f'teacher_{mode}_{prep_tag(normalize)}_target{target}.npz')
...
def run_fold(count, target, kept, epochs, teacher_importance, device,
    ...
    with open(os.path.join(count_dir, f'fold_target{target}.json'), 'w') as f:
        json.dump(result, f, indent=2, ensure_ascii=False)
    np.savez(os.path.join(count_dir, f'fold_target{target}.npz'),
    ...
    torch.save(model.state_dict(),
               os.path.join(count_dir, f'concept_dynamic_net_target{target}.pth'))
...
def aggregate_count(count, base_dir, manifest, verbose=True):
    count_dir = os.path.join(base_dir, f'channel{count}')
    files = sorted(glob.glob(os.path.join(count_dir, 'fold_target*.json')))
    if not files:
        return None
...
def main():
    ...
    print(f"test/validation候補: {len(eligible)}名 {eligible}")
