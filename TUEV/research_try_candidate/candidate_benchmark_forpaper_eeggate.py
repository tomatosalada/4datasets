...
#     - クラス別 recall (SPSW/GPED/PLED/EYEM/ARTF/BCKG)
#       ※ 部位とクラスの対応 (例: 前頭極固定 -> EYEM が上がる) を検証するため。
#         指標差が偶然でなく生理学的に説明できることの裏付けになる。
#
#   使い方:
#     python candidate_benchmark_forpaper.py                    # 既定 total=10, fixed=4, 5 seeds
#     python candidate_benchmark_forpaper.py --seeds 42 43 44 45 46
#     python candidate_benchmark_forpaper.py --procs-per-gpu 2  # 全探索と併走させる場合は下げる
#     python candidate_benchmark_forpaper.py --analyze-only     # 学習せず集計だけやり直す
# =========================================================================
import sys
import os
...
from ratio_fixdynamic_forpaper_eeggate_2 import (
...
CANDIDATES = {
    ...
    'anterior_temporal': dict(
        channels=['F7-T3', 'T3-T5', 'F8-T4', 'T4-T6'],
        group='clinical',
        rationale='側頭葉てんかんは成人の焦点性てんかんで最多。内側側頭起源の発作間欠期'
                  'てんかん性放電は前〜中側頭 (F7/T3, F8/T4) で振幅最大となる。'),

    'frontotemporal': dict(
        channels=['FP1-F7', 'F7-T3', 'FP2-F8', 'F8-T4'],
        group='clinical',
        rationale='前頭葉てんかんは焦点性てんかんで側頭葉に次いで多い。前頭下面〜側頭前部を'
                  'カバーし、焦点性発作の大半の起始領域を含む。'),

    'centrotemporal': dict(
        channels=['T3-T5', 'T4-T6', 'C3-P3', 'C4-P4'],
        group='clinical',
        rationale='中心側頭部棘波を伴う自然終息性小児てんかん (ローランドてんかん) の'
                  '棘波分布。中心-側頭移行部に最大電位をもつ。'),

    'frontopolar': dict(
        channels=['FP1-F7', 'FP1-F3', 'FP2-F8', 'FP2-F4'],
        group='clinical',
        rationale='前頭極。全般性周期性放電 (GPED)・三相波は前頭優位。眼球運動由来電位も'
                  '最大となるため TUEV の EYEM クラスに直結する。前頭ウェアラブル EEG の'
                  '実運用形態にも対応。'),

    'occipital': dict(
        channels=['T5-O1', 'P3-O1', 'T6-O2', 'P4-O2'],
        group='clinical',
    ...
    'symmetric_pairs': dict(
        channels=['F7-T3', 'F8-T4', 'C3-P3', 'C4-P4'],
        group='clinical',
        rationale='左右相同ペア×2。片側性周期性放電 (PLED) の側方性判定に必要な最小構成であり、'
                  '前方 (側頭) と後方 (中心-頭頂) の両方に対称配置している。'),

    'left_temporal': dict(
        channels=['FP1-F7', 'F7-T3', 'T3-T5', 'T5-O1'],
        group='control',
        rationale='左側頭鎖のみ。側方性が一方に偏った場合の対照 (PLED 等の片側性事象に有利、'
                  '対側事象に不利になるはず)。'),

    'right_temporal': dict(
        channels=['FP2-F8', 'F8-T4', 'T4-T6', 'T6-O2'],
        group='control',
        rationale='右側頭鎖のみ。left_temporal との対比で左右非対称性の影響を見る。'),
}
...
def analyze(csv_path, res_dir, fixed_n):
    ...
    add("  ※ 期待される対応: frontopolar -> EYEM, anterior_temporal/left_temporal -> PLED/SPSW,")
    add("     parasagittal -> ARTF が少なく BCKG/GPED 寄り。ここが一致すれば差は生理学的に説明できる。")
    add("")
...
if __name__ == '__main__':
    ...
    res_dir = os.path.join(OUTPUT_ROOT_DIR,
                           f'candidate_results_total{TOTAL}_fix{FIXED}_2')
