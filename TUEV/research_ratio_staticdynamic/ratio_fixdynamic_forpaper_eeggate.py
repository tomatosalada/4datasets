...
BATCH_SIZE = 512
DEFAULT_EPOCH = 100
DEFAULT_TOTAL_ELECTRODES = 10  # コマンドライン --total で上書き可能

LR_GATE = 2e-4
LR_CLASSIFIER = 2e-4
WEIGHT_DECAY = 0.03723  # 元コードで optimizer にハードコードされていた値
...
RESEARCH_ROOT_DIR = '/mnt/data/toshiki.ohno/TUEV_dataset/TU1v/TU1v/v2.0.1/research_ratio_fixdynamic/eeggate_result/'
...
# 固定電極の選定順 (重要度が高い順)。
# 事前学習の成果物 holdout_eval_results.txt の "Electrode Importance Ranking" を
# そのままコードに埋め込んだもの。上位から順に「固定する電極」に採用する。
IMPORTANCE_RANKING = [
    'FP2-F4',  #  1: 0.089367
    'F7-T3',   #  2: 0.069850
    'F3-C3',   #  3: 0.067045
...
# =========================================================================
# ★ Gate (動的な電極選択) の設計
#   research_number_of_electrode/test_eegnet_researchbestnum__eeggate.py の
#   改修をそのまま移植したもの。ここ以外の処理 (データ読み込み・学習ループ・
#   評価・可視化・固定/可変チャネルの割合スイープ) は変えていない。
#
...
# =========================================================================
# 1. Gate Mechanism
#   ★ test_eegnet_researchbestnum__eeggate.py から移植
# =========================================================================
...
class ConceptDynamicNet(nn.Module):
    ...
    固定電極 (fixed_indices) は常に ON なので、Gate が選ぶのは可変電極からの k 本だけ。
    ...
    def gate_features(self, x):
        """Gate に渡す EEG 特徴。x: [B, 10(time), 5(freq), 16(channel)]"""
    ...
    def _select(self, scores):
        ...
        elif self.select == 'topk' and k is not None and k <= 0:
            # 固定電極だけで目標本数に達している場合 (固定割合 100%)。
            # 可変電極は 1 本も選ばない。
